...
int main(int argc, char** argv) 
{
    ...
    int min = stoll(params[0]), max = stoll(params[1]);
	...
	for(int i = 0; i < n; ++i)
	{
		char c = Random::Value() > 0.5f ? 'o' : 'p';
		acronym += c;

		std::string resultPart;
		if (c == 'o')
		{
			resultPart = oToggle ? "oriented" : "object";
			oToggle = !oToggle;
		}
		else
		{
			resultPart = pToggle ? "please" : "programming";
			pToggle = !pToggle;
		}

		std::string space = i < n - 1 ? " " : "";
		result += resultPart + space;
	}
