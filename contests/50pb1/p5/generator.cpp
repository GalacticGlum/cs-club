...
int main(int argc, char** argv) 
{
	...
	for(int i = 0; i < n; ++i)
	{
		values[i] = static_cast<double>(Random::Range(-1000000, 1000000));
		std::cout << values[i] << " ";
	}
	...
	double mean = std::accumulate(values.begin(), values.end(), 0) / (double)n;
	double median = n % 2 != 0 ? values[(n+1)/2-1] : (values[n/2-1]+values[(n+2)/2-1])/2;
	double sigma = 0;
	for(auto& v : values)
	{
		sigma += std::pow(v - mean, 2);
	}

	sigma = std::sqrt(sigma / (n - 1));
	std::cerr << std::fixed << std::setprecision(2) << mean << " " << median << " " << sigma << "\n";
}