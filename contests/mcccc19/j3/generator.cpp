...
int main(int argc, char** argv) 
{
    ...
    long long seed = stoll(params[0]), min = stoll(params[1]), max = stoll(params[2]);
    
    long long n = Random::Range(min, max);
    long long c = Random::Range(1LL, 100LL);
    std::cout << n << " " << c << "\n";
    ...
    {
        long long x = Random::Range(-1000LL, 1000LL);
        long long y = Random::Range(-1000LL, 1000LL);
    ...
    long long dx = std::abs(minX - maxX);
    long long dy = std::abs(minY - maxY);
    long long r = 2 * (dx + dy) * c;
    std::cerr << r << "\n";
}