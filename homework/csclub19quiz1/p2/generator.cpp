...
{   
    ...
    double result = (std::sqrt(1 + std::pow(p(x-1),2)));
    memo[x] = result;
...
int main(int argc, char** argv) 
{
    ...
    if (subtask == 1)
    {
        q = Random::Range(1, 100);
        for (int i = 0; i< q; ++i)
        {
            queries.push_back(Random::Range(0, 20));
        }
    }
    else
    {
        q = Random::Range(1, 1000);
        for (int i = 0; i< q; ++i)
        {
            queries.push_back(Random::Range(0, 1000));
        }
    }
